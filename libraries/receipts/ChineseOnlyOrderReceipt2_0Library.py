...
_REPRINT_CN_TEXT: str = "xpath://font[starts-with(text(),'重复打印')]"
_REPRINT_TIME: str = "xpath://font[starts-with(text(),'重复打印')]//following-sibling::font"

# For counting items only
_CN_DISH_DESC_INDEXED: str = "xpath:(//table[2]//td[@class='description']/font)"
...
_PAID_BY_CREDIT_CARD_NOTE: str = "xpath://font[contains(text(),'付信用卡')]"


@library(scope='GLOBAL')
class ChineseOnlyOrderReceipt2_0Library(OrderReceiptLibraryBase):
    ...
    def order_receipt_details_should_include(self, html_obj: HTML, res_en_name: str = None, res_addr1: str = None, res_addr2: str = None, res_city: str = None, res_state: str = None, res_zipcode: str = None, res_phone: str = None,
                                       cn_order_type: str = None, order_number: str = None, cx_name: str = None, cx_phone1: str = None, cx_phone2: str = None,
        ...
        if cn_order_type is not None:
            sa.assert_equal(actual=html_obj.get_text_of_element(locator=_CN_ORDER_TYPE).strip(), exp=cn_order_type,
                            desc="CN Order Type.")
    ...
    @keyword
    def reprint_details_should_should_be_displayed_in_the_order_receipt(self, html_obj: HTML, reprint_number: str, reprint_cn_text: str = None):
        sa = self.asserts.SoftAssert()
        if reprint_cn_text is not None:
            sa.assert_equal(actual=html_obj.get_text_of_element(locator=_REPRINT_CN_TEXT),
                            exp=f"{reprint_cn_text}#{reprint_number}", desc="Reprint CN text.")
